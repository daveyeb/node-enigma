...
function Enigma(...config) {
    if (config.length < 4) {
        throw new Error("Enigma machine configuration is not setup right! Must be three or more wheels assigned/configured!")
    }
...
Enigma.prototype.encode = function(plaintext) {
    ...
    var ciphertext = [...plaintext.toUpperCase()].map((pt) => {
        ...
        var code = this.config.match(/\d*\.\d*\.\d*/g).join("").split(".").map((c) => parseInt(c, 10))
        var trip = 0
        var rtrip = midPathway

        pt = plugboard[getWheels(0).indexOf(pt)]
        while (trip <= pathway) {
            pt = trip < rotorslength ? rotors[trip][getSignal(trip, codeIndices)(pt, code)] :
                getWheels(0)[rotors[rtrip].indexOf(getWheels(0)[getSignal(trip, codeIndices)(pt, code)])]

            if (!(trip < rotorslength)) {
                rtrip--;
            }
            trip++;
        }

        pt = plugboard[getSignal(trip, codeIndices)(pt, code)]
        return pt;
    });
