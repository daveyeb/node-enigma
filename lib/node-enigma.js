...
const SIGNALS = [
  (input, code) => {
    var signal = (getIndex(input) + code[2]) % NUMALPHA;
    return signal < 0 ? signal + NUMALPHA : signal;
  },
  (input, code) => {
    var signal = (getIndex(input) + (code[1] - code[2])) % NUMALPHA;
    return signal < 0 ? signal + NUMALPHA : signal;
  },
  (input, code) => {
    var signal = (getIndex(input) + (code[0] - code[1])) % NUMALPHA;
    return signal < 0 ? signal + NUMALPHA : signal;
  },
  (input, code) => {
    var signal = (getIndex(input) - code[0]) % NUMALPHA;
    return signal < 0 ? signal + NUMALPHA : signal;
  },
  (input, code) => {
  ...
  (input, code) => {
    var signal = (getIndex(input) + code[0]) % NUMALPHA;
    return wheels["etw"][signal < 0 ? signal + NUMALPHA : signal];
  },
  (input, code) => {
    var signal = (getIndex(input) + (code[1] - code[0])) % NUMALPHA;
    return wheels["etw"][signal < 0 ? signal + NUMALPHA : signal];
  },
  (input, code) => {
    var signal = (getIndex(input) + (code[2] - code[1])) % NUMALPHA;
    return wheels["etw"][signal < 0 ? signal + NUMALPHA : signal];
  },
  (input, code) => {
    var signal = (getIndex(input) - code[2]) % NUMALPHA;
...
function increment() {
  ...
  if (rotors[1].turnover.split("").includes(etw[code[1]]) && !this.flag) {
    code[2] = (code[2] + 1) % NUMALPHA;
    code[1] = (code[1] + 1) % NUMALPHA;
    code[0] = (code[0] + 1) % NUMALPHA;
    this.flag = true;
    this.code = code;
    return;
  }
